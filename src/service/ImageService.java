...
public class ImageService {
	...
	public static BufferedImage getGreenViewImage(GreenView view){
		...
		double yardToPixelRatio = view.getViewSize()/w;
		// draw flying ball.
		if(view.getFlyingPosition()!=null){
//			double xYard= view.getFlyBallToCenterDistanceX();
//			double zYard= view.getFlyBallToCenterDistanceZ();
			
			//System.out.println(view.getFlyBallAbsoluteX() + " " + view.getFlyBallAbsoluteZ());
			
			int imageX = (int)(view.getFlyBallAbsoluteX() /yardToPixelRatio);
			int imageY = h - (int)(view.getFlyBallAbsoluteZ() /yardToPixelRatio); // reverse Y 

			Ellipse2D.Double circle = new Ellipse2D.Double(imageX-4,imageY-4, 8, 8); // shift center
			g2d.fill(circle);
		}
		ArrayList<Position> absPositions = view.getLandedBallsAbsolute();
		if(absPositions!=null){
			for(Position p : absPositions){
				int imageX = (int)(p.x /yardToPixelRatio);
				int imageY = h - (int)(p.z /yardToPixelRatio); // reverse Y
				g2d.setPaint(Color.BLUE); // landed ball color
				Ellipse2D.Double circle = new Ellipse2D.Double(imageX-4,imageY-4, 8, 8); // shift center 
	...
	public static void cacheImages() {
		...
		try {
			...
			bufferedImage =ImageIO.read(new File(IMAGE_ROOT + "HitMeBubble.png"));
			plotElementMap.put(HitMeBubble.class, bufferedImage);
